...
export const CodeShowcase = () => {
  return (
    <section className="relative flex flex-col items-center justify-center py-10 md:py-20 px-4 z-[20]">
      <motion.div
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
      ...
      >
        ...
        <div className="relative w-full flex items-center justify-center h-auto min-h-[350px] md:h-[500px]">
          
          {/* Layer 1: Background Glass */}
          <motion.div 
            variants={slideInFromRight(1.2)}
            className="hidden md:block absolute w-[80%] h-[120%] bg-white/5 backdrop-blur-3xl rounded-[40px] border border-white/10 -rotate-3 z-0"
          ></motion.div>

          {/* Layer 2: Middle Glass */}
          <motion.div 
            variants={slideInFromLeft(1.0)}
            className="hidden md:block absolute w-[90%] h-[110%] bg-white/5 backdrop-blur-2xl rounded-[30px] border border-white/20 rotate-2 z-10 shadow-2xl"
          ></motion.div>

          {/* Layer 3: Top Glass */}
          <motion.div
            variants={slideInFromRight(0.8)}
            className="relative w-full bg-white/10 backdrop-blur-xl rounded-2xl p-4 sm:p-8 md:p-12 border border-white/30 z-20 shadow-[0_0_50px_rgba(255,255,255,0.1)] flex flex-col gap-6"
          >
            {/* Window Controls */}
            ...
            {/* Code Content */}
            <pre className="font-mono text-[11px] sm:text-sm md:text-xl leading-relaxed sm:leading-loose overflow-x-auto scrollbar-hidden">
